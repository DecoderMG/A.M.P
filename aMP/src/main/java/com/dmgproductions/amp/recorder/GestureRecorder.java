package com.dmgproductions.amp.recorder;
...
public class GestureRecorder implements SensorEventListener {
	...
	final int MIN_GESTURE_SIZE = 40;
	float THRESHOLD = 2;
	SensorManager sensorManager;
	boolean isRecording;

	public int stepsSinceNoMovement;
	...
	private float calcVectorNorm(float[] values) 
	{
		float norm = (float) Math.sqrt(
				values[SensorManager.DATA_X] * values[SensorManager.DATA_X] 
				+ values[SensorManager.DATA_Y] * values[SensorManager.DATA_Y] 
				+ values[SensorManager.DATA_Z] * values[SensorManager.DATA_Z]
						) - 9.9f;
	...
	public void callRecognition()
	{
		stepsSinceNoMovement = 101;
	}
	...
	public void onSensorChanged(SensorEvent sensorEvent) {
		...
		switch (recordMode) 
		{
			case MOTION_DETECTION:
				...
				if (stepsSinceNoMovement >= 50) 
				{
					if(gestureValues == null)
					{
						System.out.print("Array is empty");
					}
					//System.out.println("Length is: " + String.valueOf(gestureValues.size() - 10));
					else if (gestureValues.size() - 10 > MIN_GESTURE_SIZE) 
					{
